          ...
          <div className="flex-1 bg-gray-100 rounded-full h-2">
            <div
              className={`h-2 rounded-full ${
                ...
                application.matchScore >= 40 ? 'bg-yellow-500' : 'bg-red-500'
              }`}
              style={{ width: `${application.matchScore}%` }}
            />
          </div>
          <span className="text-xs font-medium text-gray-600">{application.matchScore}%</span>
        </div>
      )}

      <div className="flex items-center gap-2 mt-2 text-sm">
        <button onClick={() => onEdit(application)} className="text-blue-600 hover:underline">
          Edit
        </button>
        <button onClick={() => onDelete(application._id)} className="text-red-600 hover:underline">
          Delete
        </button>
        {application.jobDescription && (
          ...
          >
            {scoring ? 'Scoring...' : application.matchScore != null ? 'Re-score' : 'AI Match Score'}
          </button>
        )}
      </div>
    </div>
